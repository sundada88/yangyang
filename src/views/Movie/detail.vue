    ...
    >
      <div class="detail_list">
        <div
          ...
          :style="{ 'background-image' : detailMovie.img ? 'url('+ detailMovie.img.replace(/w\.h/,'148.208') +')' : ''}"
        ></div>
...
export default {
  ...
  mounted () {
    window.console.log(this.movieId)
    this.$axios.get(`/api/detailMovie?movieId=${this.movieId}`).then(res => {
      ...
      if (msg === 'ok') {
        ...
        this.$nextTick(() => {
          new Swiper(this.$refs.detail_player, { /* eslint-disable-line */
            slidesPerView: 'auto',
            freeMode: true,
            freeModeSticky: true
          })
